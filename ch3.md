...
但是，我们将从您最可能熟悉的两大类数据库：传统关系型数据库与很多所谓的“NoSQL”数据库开始，通过介绍它们的**存储引擎**来开始本章的内容。我们会研究两大类存储引擎：**日志结构（log-structured）** 的存储引擎，以及**面向页（page-oriented）** 的存储引擎（例如B树）。
...
> 范围查询这个好像经常考，如果索引/数据本身是有序的，那么就很方便。否则就要全盘
=======
> 确实，这里就是排序的作用，清楚查找出范围。
...
> bloom filter. It will tell you whether the key exists or not
...
> 确实有道理，毕竟我们有了虚拟内存，可以把硬盘里经常用的数据通过LRU缓存在内存分页里。
>
> 所以开销花在了encoding,≈序列化？
>
> https://hstore.cs.brown.edu/papers/hstore-lookingglass.pdf
>
>  We found that buffer management and locking operations are the most significant contributors to system overhead
...
> bitmap,位图。如果 n 非常小（例如，国家/地区列可能有大约200个不同的值），则这些位图可以每行存储一位。但是，如果n更大，大部分位图中将会有很多的零（我们说它们是稀疏的）。如果有这个值，则标为1
...
> use simd to parallelism
>
> single instruction, multiple data
>
> http://www1.cs.columbia.edu/~kar/pubsk/simd.pdf
