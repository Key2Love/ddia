...
> 首先，附上[github开源中文link](https://github.com/Vonng/ddia)，我是买的英文版实体书，配合这个库一起看。好处在于既能直接理解作者的意思，同时做笔记的时候也能根据中文快速定位，同时中文库里有reference链接，更方便。
...
> 游戏客户端通常是**计算密集型**（compute-intensive）的应用，而现今有很多应用程序是**数据密集型**（data-intensive），比如微博。
>
> 多组件的数据系统架构，和游戏服务器逐步抽象各个服务的方式有些像，除了DB等部分，也会使用Redis这样的中间件做内存缓存，拆分聊天服务器、战斗服务器等等。
...
> a **data bank** or **databank** is a repository of information on one or more subjects – a [database](https://en.wikipedia.org/wiki/Database) – that is organized in a way that facilitates local or remote information retrieval and is able to process many continual queries over a long period of time. 
>
> Reference 1: 关系型数据模型的开山之作
...
​	当你将多个工具组合在一起提供服务时，服务的接口或API通常向客户端隐藏这些实现细节。现在，你基本上已经使用较小的通用组件创建了一个全新的、专用的数据系统。这个新的复合数据系统可能会提供特定的保证，例如：缓存在写入时会作废或更新，以便外部客户端获取一致的结果。现在你不仅是应用程序开发人员，还是数据系统设计人员了。
...
> Reference 2 is a paper about Simple Testing Can Prevent Most Critical Failures
>
> https://www.usenix.org/system/files/conference/osdi14/osdi14-paper-yuan.pdf
>
> The abstract is funny, and they make a tool to check bugs
...
>
> https://netflixtechblog.com/the-netflix-simian-army-16e57fbab116?gi=f976a150ecf9
...
> We should read the english version here, because the difference between fault and failure
>
> fault:an imperfection in a device or machine
>
...
[^ii]: Fan-out扇出：从电子工程学中借用的术语，它描述了输入连接到另一个门输出的逻辑门数量。 输出需要提供足够的电流来驱动所有连接的输入。 在事务处理系统中，我们使用它来描述为了服务一个传入请求而需要执行其他服务的请求数量。
...
> 这块挺有意思，个人觉得是这一章有意思的。
>
> 两篇blog，https://dirtysalt.github.io/html/timeline-at-scale-in-twitter.html
>
...
> 这也是大数据和基础业务、数仓和数据库、离线计算和业务sql的区别，关注不同。
...
> 这块可以解释Spark的web gui界面为啥要搞那么多xx th百分位了，毕竟是短板原理。最耗时的完成了，所有的也就完成了。
