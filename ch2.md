...
[^i]: **impedance mismatch**一个从电子学借用的术语。每个电路的输入和输出都有一定的阻抗（交流电阻）。当你将一个电路的输出连接到另一个电路的输入时，如果两个电路的输出和输入阻抗匹配，则连接上的功率传输将被最大化。阻抗不匹配会导致信号反射及其他问题。

> [https://devblogs.microsoft.com/oldnewthing/20180123-00/?p=97865#:~:text=In%20electrical%20engineering%2C%20an%20impedance,what%20it%20means%20at%20Microsoft.](https://devblogs.microsoft.com/oldnewthing/20180123-00/?p=97865#:~:text=In electrical engineering%2C an impedance,what it means at Microsoft.)
>
> 一篇关于impedance mismatch的blog
...
> 这块主要是介绍变更Mysql table Schema的开源工具
>
...
> Why we should keep doc small?
>
> https://docs.atlas.mongodb.com/schema-suggestions/reduce-document-size/
>
> As we all know, memory is more local than disk. So Storing large documents in your database can lead to excessive RAM and bandwidth usage. MongoDB keeps frequently accessed data, referred to as the [working set](https://docs.mongodb.com/manual/reference/glossary/#term-working-set), in RAM. When the working set grows beyond the RAM allotment, performance is degraded as data must be retrieved from disk instead.
>
> 如果doc很小，MongoDB可以把这部分数据保存在memory
>
...
>
> ![image-20201011141557686](/Users/admin/Library/Application Support/typora-user-images/image-20201011141557686.png)
>
> 

在[第3章](ch3.md)将还会看到更多关于局部性的内容。

> There is a brief answer about what does locality mean in hadoop](https://www.quora.com/What-does-the-term-data-locality-mean-in-Hadoop)
>
> It seems to be not so exact about locality in this book 
...
最后，声明式语言往往适合并行执行。现在，CPU的速度通过核心(core)的增加变得更快，而不是以比以前更高的时钟速度运行【31】。命令代码很难在多个核心(core)和多个机器之间并行化，因为它指定了指令必须以特定顺序执行。声明式语言更具有并行执行的潜力，因为它们仅指定结果的模式，而不指定用于确定结果的算法。在适当情况下，数据库可以自由使用查询语言的并行实现【32】。

> http://www.gotw.ca/publications/concurrency-ddj.htm Reference31主要是cpu发展的解释，因为从单核性能提升转向了多核，所以我们要用好concurrency
